...
description: >-
  This page publishes the tamatebako release root of trust: the OpenPGP
  fingerprint, the armored public key, what carries a signature, and
  the verification of a downloaded artifact with tebako-pkg verify.
---
...
The armored public key is served at a stable URL: link:/.well-known/tebako-key.asc[tebako-key.asc]. Its content is reproduced here so the two copies can be compared.
...
Releases older than the go-live carry no `.asc` files. The following command on such an artifact verifies its integrity instead, through the per-asset digest sidecar described in link:/docs/guides/verify/[Verify integrity]. Unsigned artifacts stay first-class everywhere: they install and run with a warning, while verification of anything signed is always strict — a bad signature is a refusal, never a warning.
