...
artifacts it names (spec 04 §2). A `file://` registry is the local form:
...
payload the artifact's SHA-256 rides the reference itself as the
`?sha256=` pin:
...
`acme-tool`, `tebako shim use` pins versions, `tebako cache list` shows
what is cached.
